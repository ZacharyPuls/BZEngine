...
void main()
{
    ...
    if (HasNormalMap != 0)
    {
        ...
        V = normalize(fTangentViewPosition - fTangentFragPosition);
        H = normalize(fTangentLightPosition + V);
        G = max(0, dot(fTangentLightPosition, N));
    }
    else
    {
        N = normalize(fWorldNormal);
        V = normalize(fViewPosition - fWorldPosition);
        H = normalize(fLightPosition + V);
        G = max(0, dot(fLightPosition, N));
    }
